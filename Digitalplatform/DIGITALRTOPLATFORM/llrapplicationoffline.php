...
if(isset($_POST['submit']))
{
	...
	move_uploaded_file($_FILES["photo"]["tmp_name"],"llrphoto/".$filename);
	
	$mfilename = rand().$_FILES["identificationmarks"]["name"];
	move_uploaded_file($_FILES["identificationmarks"]["tmp_name"],"llrmarksheet/".$mfilename);
	
	$document = rand().$_FILES["document"]["name"];
	move_uploaded_file($_FILES["document"]["tmp_name"],"llrdocument/".$document);
	...
	if(isset($_GET['editid']))
	{
		...
		{
			echo "<script>alert(' License updated successfully');</script>";
	...
	{
		$sql="INSERT INTO llr (llrtype, enrollerid, stateid, cityid,vehicletypeid,name,swd,Photo,paddr,taddr,paddrduration,
		dob,birthplace,qualification,identificationmarks,bloodgroup,
		startdate,enddate
		,note,document,status)VALUES 
		('','$_SESSION[enrollerid]','$_POST[stateid]','$_POST[cityid]','$vehicletypes','$_POST[name]','$_POST[swdof]',
		'$filename','$_POST[paddr]','$_POST[taddr]','$_POST[paddrduration]','$_POST[dob]','$_POST[birthplace]',
		'$_POST[qualification]','$mfilename','$_POST[bloodgroup]',
		'$_POST[startdate]','$_POST[enddate]','$_POST[note]','$document','Pending')";
		$qsql = mysqli_query($con,$sql);
		if(mysqli_affected_rows($con) == 1)
		{
			echo "<script>alert('LLR Form entry done successfully.. The page will redirect to Payment gateway..');</script>";
			$insid = mysqli_insert_id($con);
			echo "<script>window.location='payment.php?insid=$insid&paidfor=LLR';</script>";
		}
		else
		{
			echo mysqli_error($con);
...
<form action="" method="post" enctype="multipart/form-data"  onsubmit="return validateform()">
    ...
    <div class="form-group">
      <label for="email">LLR for <span class="classvalidate" id="idvehicletypes" ></span> <span style="color:red;">(Press ctrl to select multiple values)</span>:</label>
	  	<select class="form-control" id="vehicletypes" placeholder="Enter vehicle type" name="vehicletypes[]" multiple="multiple">
			<?php
...
?>	
	...
	<div class="form-group">
      ...
      <input type="date" class="form-control" id="dob" placeholder="Enter Date Of Birth" name="dob" value="<?php echo $rsedit['dltype']; ?>" max="<?php echo date('Y-m-d', strtotime("-18 years")); ?>" >
    </div>
	<div class="form-group">
	...
	<div class="form-group">
      ...
      <input type="text" class="form-control" id="qualification" placeholder="Enter Qualification" name="qualification" value="<?php echo $rsedit['qualification']; ?>">
    </div>
	<div class="form-group">
...
function validateform()
{
	...
	if(document.getElementById("vehicletypes").value=="")
	{
		document.getElementById("idvehicletypes").innerHTML="Vehicle type must be selected...";
		validateform="false";
	}
